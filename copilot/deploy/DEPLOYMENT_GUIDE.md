...
This guide covers deploying TERRA to Snowpark Container Services (SPCS).
...
```sql
-- Create role for TERRA users
CREATE ROLE IF NOT EXISTS TERRA_USER;

-- Grant service access
GRANT USAGE ON SERVICE CONSTRUCTION_GEO_DB.CONSTRUCTION_GEO.terra_service TO ROLE TERRA_USER;

-- Grant database access
GRANT USAGE ON DATABASE CONSTRUCTION_GEO_DB TO ROLE TERRA_USER;
GRANT USAGE ON SCHEMA CONSTRUCTION_GEO_DB.CONSTRUCTION_GEO TO ROLE TERRA_USER;
GRANT SELECT ON ALL VIEWS IN SCHEMA CONSTRUCTION_GEO_DB.CONSTRUCTION_GEO TO ROLE TERRA_USER;

-- Grant Cortex Search access
GRANT USAGE ON CORTEX SEARCH SERVICE SITE_DOCUMENTS_SEARCH TO ROLE TERRA_USER;

-- Assign role to user
GRANT ROLE TERRA_USER TO USER <username>;
```
